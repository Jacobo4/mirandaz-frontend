...
const EventProfilePage = () => {
    ...
    const [bar, setBar] = useState([[0], [100], [4], ['nombre jaksan'], ['descripciones'], ['https://support.content.office.net/es-es/media/2c8399b3-5194-4b4a-ad9c-04405673f8c2.jpg']]);
    ...
    const imagesDivs = party.map((image, i) => (
        <Card key={image[0]} imageUrl='https://support.content.office.net/es-es/media/2c8399b3-5194-4b4a-ad9c-04405673f8c2.jpg' title={`Fiesta ${image[1]}`} description={image[6]}
            actions={(
    ...
    return <div className="container grid gap-8">
        <div className="rounded overflow-hidden shadow-md grid grid-flow-col gap-4 max-w-2xl m-auto p-4">
            <div className={"w-32 m-auto"}>
                ...
                {(bar) && <Image src='https://support.content.office.net/es-es/media/2c8399b3-5194-4b4a-ad9c-04405673f8c2.jpg'  width={100} height={100} className={"rounded-full"} alt={""}/> }
            </div>
            {(bar) && 
            <figcaption className={"grid gap-2 content-start"}>
                <h6 className={"bold text-lg"}>{query.idEventCenter}</h6>
                <h6 className={"text-xs font-light "}>{bar[0][3]}</h6>
                <h6 className={"text-xs font-light "}>Bogotá</h6>
                <p className={"text-xs"}>{bar[0][4]}</p>
            </figcaption>}
        </div>
