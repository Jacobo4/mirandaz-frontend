    ...
    return <div className="container grid gap-8">
        <div className="rounded overflow-hidden shadow-md grid grid-flow-col gap-4 max-w-2xl m-auto p-4">
            <div className={"w-32 m-auto"}>
                {(user!= undefined) ?  <Image src={user[0][10]}  layout='fill' className={"rounded-full"} alt={""}/> : <div className={"w-32 h-32 rounded-full bg-gray-300"}/>}
               
            </div>
            <figcaption className={"grid gap-2 content-start"}>
                <h6 className={"bold text-lg"}>{query.idUser}</h6>
                <h6 className={"text-xs font-light "}>{user[0][4]}</h6>
                <h6 className={"text-xs font-light "}>{user[0][6]}</h6>
                <p className={"text-xs"}>{user[0][7]}</p>
            </figcaption>
        </div>
