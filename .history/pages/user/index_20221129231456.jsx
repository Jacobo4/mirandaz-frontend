...
const CurrentUserProfilePage = () => {
    ...
    const [myUser, setMyUser] = useState([[1], ['si'], [1], [3], ['nombre de ususrio'], ['correo@correo.com'], ['estudiante'], [false], ['https://ph-files.imgix.net/81c8176a-1b00-4f10-b60f-2ab2729d0a14.png?auto=format']]);
    ...
    const imagesDivs = data.map((image, i) => (
        <Card key={image[0]} imageUrl={image[5]} title={image[3]} description={image[4]} 
            actions={(
                <>
                    <Link href={`/user/persona${image[0]}/fiesta${i}`}><a className="btn btn-secondary">Visitar</a></Link>
                </>
            )}
        />
    ));
    ...
    return <div className="container grid gap-8">
        <div className="rounded overflow-hidden shadow-md grid grid-flow-col gap-4 max-w-2xl m-auto p-4">
            <div className={"w-32 m-auto"}>
                <Image src='https://ph-files.imgix.net/81c8176a-1b00-4f10-b60f-2ab2729d0a14.png?auto=format' width={100} height={100}
